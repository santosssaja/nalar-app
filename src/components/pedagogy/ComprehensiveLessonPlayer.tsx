...
import { BookOpen, Sparkles, Binary, Compass, CheckCircle2 } from "lucide-react";
...
function ComprehensiveLessonPlayerInner({ lesson }: { lesson: ComprehensiveLesson }) {
  ...
  const [simVars, setSimVars] = useState<Record<string, number>>(lesson.explore.initialVariables);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-6 space-y-8 animate-fade-in text-neutral-100">
          ...
          <section className="space-y-4">
            ...
            {lesson.guidedPractice.map((gp) => (
              <div key={gp.id} className="p-6 rounded-3xl bg-neutral-900 border border-neutral-800 space-y-4">
                ...
                <FourTierHintDrawer
                  hints={gp.hints}
                  onHintUnlocked={(lvl) => setHintsUsedCount((p) => Math.max(p, lvl))}
                />
              </div>
            ))}
          </section>
      ...
      {/* MODE 3: MASTER (Evaluation Matrix) */}
      {activeMode === "master" && (
        <div className="space-y-6 animate-fade-in">
          <MasteryCheckView
            topicId={lesson.id}
