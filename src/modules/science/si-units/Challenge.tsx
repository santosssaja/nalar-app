...
export function Challenge({ state, onComplete }: ChallengeProps) {
  ...
  const currentCh = siUnitsLesson.challenges[activeChallengeIdx];
    ...
    <div className="w-full bg-neutral-900 border border-neutral-800 rounded-2xl p-5 space-y-4 shadow-lg">
      <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-white">
            Tantangan {activeChallengeIdx + 1} dari {siUnitsLesson.challenges.length}
          </h3>
        </div>
        <div className="flex gap-1.5">
          {siUnitsLesson.challenges.map((ch, idx) => (
            <button
              key={ch.id}
              type="button"
              onClick={() => {
      ...
      {completedList.includes(currentCh?.id || "") ? (
        ...
        <div className="flex items-center justify-between p-3 bg-indigo-500/10 border border-indigo-500/30 rounded-xl text-xs text-indigo-300">
          <span>Nilai konversi terpenuhi!</span>
          <button
            type="button"
        ...
        <div className="flex items-center justify-between text-xs pt-1">
          ...
          <span className="flex items-center gap-1 text-neutral-500">
            <AlertCircle className="w-3.5 h-3.5" /> Sesuaikan parameter pengukuran di atas
          </span>
        </div>
      )}
    </div>
