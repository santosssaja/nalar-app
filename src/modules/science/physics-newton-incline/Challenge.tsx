...
export function Challenge({ currentState }: ChallengeProps) {
  ...
  const challenges = inclineLesson.challenges;
  const currentChallenge = challenges[activeIndex];
  const challengeId = `challenge-incline-${activeIndex + 1}`;
  const isAlreadySolved = progress.solvedChallenges.includes(challengeId);

  const handleCheckSolution = () => {
    const varsMap: Record<string, number> = {
      angleDeg: currentState.angleDeg,
      mass: currentState.mass,
      frictionCoeff: currentState.frictionCoeff,
      gravity: currentState.gravity,
      inclineLength: currentState.inclineLength,
    };
    ...
    if (passed) {
      ...
      completeChallenge({
        topicSlug: "physics-newton-incline",
        challengeId,
        xpReward: currentChallenge.xpReward || 50,
      });

      speakText(currentChallenge.successMessage || "Luar biasa! Analisis hukum Newton kamu tepat sekali.");
    } else {
      setIsSuccess(false);
      ...
      if (nextFails >= 2 && !hint) {
        ...
        speakText("Belum tepat. Coba sesuaikan sudut kemiringan atau koefisien gesekan.");
      }
    }
  };
  ...
  return (
    <div
      role="region"
      aria-label="Tantangan Logika Bidang Miring"
    ...
    >
      ...
      {!isSuccess && failedAttempts > 0 && (
        <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-xs text-rose-300 flex items-center gap-2 animate-fade-in">
          ...
          <span>Kondisi belum terpenuhi. Periksa kembali gaya dan sudut luncur.</span>
        </div>
      )}
