...
import { trigLesson } from "./content";
import { TrigCanvasState } from "./engine";

interface ChallengeProps {
  currentState: TrigCanvasState;
}
...
export function Challenge({ currentState }: ChallengeProps) {
  ...
  const challenges = trigLesson.challenges;
  const currentChallenge = challenges[activeIndex];
  const challengeId = `challenge-trig-${activeIndex + 1}`;
  ...
  const handleCheckSolution = () => {
    ...
    if (passed) {
      ...
      completeChallenge({
        topicSlug: "math-trig-unit-circle",
        challengeId,
        xpReward: currentChallenge.xpReward || 50,
      });

      speakText("Luar biasa! Posisi sudut lingkaran satuan tercapai dengan tepat.");
    } else {
      setIsSuccess(false);
      ...
      if (nextFails >= 2 && !hint) {
        ...
        speakText("Posisi sudut belum memenuhi target. Coba sesuaikan slider atau seret titik.");
      }
    }
  };
  ...
  return (
    <div
      role="region"
      aria-label="Tantangan Logika Lingkaran Satuan Trigonometri"
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-neutral-100 text-sm">
            Tantangan Logika {activeIndex + 1} dari {challenges.length}
          </h3>
        </div>
        <div className="flex items-center gap-1.5">
          {challenges.map((_, idx) => {
            const isDone = progress.solvedChallenges.includes(`challenge-trig-${idx + 1}`);
      ...
      {hint && (
        <div className="p-3.5 rounded-xl bg-indigo-950/40 border border-indigo-800/60 text-xs text-indigo-200 flex items-start gap-2.5 animate-fadeIn">
          <Sparkles className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <span className="font-bold text-indigo-300">Petunjuk Konseptual Nai:</span>
      ...
      {isSuccess && (
        <div className="p-3.5 rounded-xl bg-emerald-950/50 border border-emerald-800/70 text-xs text-emerald-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">Benar! Pemahaman geometri trigonometri Anda tepat.</span>
          </div>
      ...
      {!isSuccess && failedAttempts > 0 && (
        <div className="flex items-center gap-2 text-xs text-rose-400 font-medium px-1">
          ...
          <span>Nilai sudut belum tepat ({failedAttempts}x percobaan). Terus eksplorasi!</span>
        </div>
      )}
