...
interface ChallengeProps {
  ...
  topicSlug: string;
  currentState: ModularClockState;
}
...
export function Challenge({ challenges, topicSlug, currentState }: ChallengeProps) {
  ...
  const handleCheckSolution = () => {
    const varsMap: Record<string, number> = {
      ...
      hourA: currentState.hourA,
      hourB: currentState.hourB,
    };
    ...
    if (passed) {
      ...
      completeChallenge({
        ...
        badgeToUnlock: activeIndex === 2 ? "intuition-builder" : undefined,
      });
      ...
      if (nextFails >= 2 && !hint) {
        ...
        speakText("Nilai belum memenuhi target. Coba periksa parameter jam dan coba lagi.");
      }
    }
  };
  ...
  return (
    <div
      role="region"
      aria-label="Tantangan Logika Aritmetika Jam"
    ...
    >
      ...
      <div className="flex items-center justify-between pb-3 border-b border-neutral-800">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-sm">Teka-Teki Aritmetika Modular</h3>
        </div>
      ...
      {isSuccess ? (
        ...
        <div className="p-3 rounded-xl bg-rose-950/40 border border-rose-800 text-rose-200 text-xs flex items-center gap-2">
          ...
          <span>Kondisi belum terpenuhi ({failedAttempts}x percobaan). Periksa kembali sisa pembagian dan geser parameter.</span>
        </div>
      ) : null}

      {/* Progressive Hint */}
      {hint && (
        <div className="p-3 rounded-xl bg-amber-950/50 border border-amber-700 text-amber-200 text-xs space-y-1">
          <span className="font-bold flex items-center gap-1.5 text-amber-300">
            <HelpCircle className="w-3.5 h-3.5" /> Bantuan Tutor:
          </span>
      ...
      <div className="pt-2 flex items-center gap-2.5">
        <button
          ...
          className="flex-1 py-2.5 px-4 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-bold text-xs tracking-wide shadow-lg shadow-amber-600/20 transition focus-visible:ring-2 focus-visible:ring-amber-400"
        >
        ...
        <button
          ...
          aria-label="Minta petunjuk bertahap"
        >
