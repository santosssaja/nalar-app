...
interface ChallengeProps {
  currentState: WaveSimulatorState;
}
...
export function Challenge({ currentState }: ChallengeProps) {
  ...
  const challenges = waveLesson.challenges;
  const currentChallenge = challenges[activeIndex];
  const challengeId = `challenge-wave-${activeIndex + 1}`;
  ...
  const handleCheckSolution = () => {
    const varsMap: Record<string, number> = {
      ...
      wavelength: currentState.wavelength,
      phaseDeg: currentState.phaseDeg,
      wave2Amplitude: currentState.wave2Amplitude,
      wave2PhaseDeg: currentState.wave2PhaseDeg,
    };
    ...
    if (passed) {
      ...
      completeChallenge({
        topicSlug: "physics-wave-simulator",
        challengeId,
        xpReward: currentChallenge.xpReward || 50,
      });

      speakText(currentChallenge.successMessage || "Luar biasa! Target parameter gelombang tercapai.");
    } else {
      setIsSuccess(false);
      ...
      if (nextFails >= 2 && !hint) {
        ...
        speakText("Belum tepat. Coba sesuaikan frekuensi, panjang gelombang, atau beda fase.");
      }
    }
  };
  ...
  return (
    <div
      role="region"
      aria-label="Tantangan Logika Gelombang"
    ...
    >
      ...
      {!isSuccess && failedAttempts > 0 && (
        <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-xs text-rose-300 flex items-center gap-2 animate-fade-in">
          ...
          <span>Kondisi gelombang belum terpenuhi. Periksa kembali nilai parameter.</span>
        </div>
      )}
