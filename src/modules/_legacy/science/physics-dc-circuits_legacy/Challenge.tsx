...
export function Challenge({ currentState }: ChallengeProps) {
  ...
  const challenges = dcCircuitLesson.challenges;
  const currentChallenge = challenges[activeIndex];
  const challengeId = `challenge-circuit-${activeIndex + 1}`;
  ...
  const handleCheckSolution = () => {
    const varsMap: Record<string, number> = {
      ...
      r2: currentState.r2,
      r3: currentState.r3,
      isSwitchClosed: currentState.isSwitchClosed ? 1 : 0,
    };
    ...
    if (passed) {
      ...
      completeChallenge({
        topicSlug: "physics-dc-circuits",
        challengeId,
        xpReward: currentChallenge.xpReward || 40,
      });

      speakText(currentChallenge.successMessage || "Luar biasa! Target rangkaian listrik tercapai.");
    } else {
      setIsSuccess(false);
      ...
      if (nextFails >= 2 && !hint) {
        ...
        speakText("Arus atau daya belum sesuai target misi. Coba periksa kembali nilai tegangan dan hambatan.");
      }
    }
  };
  ...
  return (
    <div
      ...
      aria-label="Tantangan Logika Rangkaian DC"
    >
      <div className="flex items-center justify-between">
        ...
        {isAlreadySolved && (
          <span className="text-xs bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded-full flex items-center gap-1">
            <CheckCircle2 className="w-3 h-3" /> Selesai (+{currentChallenge.xpReward || 40} XP)
          </span>
        )}
      </div>
      ...
      {failedAttempts > 0 && !isSuccess && (
        <div className="p-2.5 bg-rose-950/30 border border-rose-800/50 rounded-lg text-xs text-rose-300 flex items-center gap-2">
          ...
          <span>Nilai belum sesuai target. Pastikan sakelar tertutup dan periksa kembali Hukum Ohm.</span>
        </div>
      )}
      ...
      {isSuccess && (
        <div className="p-3 bg-emerald-950/40 border border-emerald-800/60 rounded-lg text-xs text-emerald-300 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">{currentChallenge.successMessage}</span>
          </div>
