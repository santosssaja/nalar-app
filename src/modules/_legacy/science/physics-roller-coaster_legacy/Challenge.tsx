...
interface ChallengeProps {
  currentState: RollerCoasterState;
}
...
export function Challenge({ currentState }: ChallengeProps) {
  ...
  const challenges = rollerCoasterLesson.challenges;
  const currentChallenge = challenges[activeIndex];
  const challengeId = `challenge-coaster-${activeIndex + 1}`;
  ...
  const handleCheckSolution = () => {
    const varsMap: Record<string, number> = {
      ...
      loopRadius: currentState.loopRadius,
      mass: currentState.mass,
      gravity: currentState.gravity,
      initialVelocity: currentState.initialVelocity,
    };
    ...
    if (passed) {
      ...
      completeChallenge({
        topicSlug: "physics-roller-coaster",
        challengeId,
        xpReward: currentChallenge.xpReward || 50,
      });

      speakText(currentChallenge.successMessage || "Luar biasa! Perhitungan energi mekanik kamu akurat.");
    } else {
      setIsSuccess(false);
      ...
      if (nextFails >= 2 && !hint) {
        ...
        speakText("Belum tepat. Coba periksa kembali ketinggian bukit awal atau radius loop.");
      }
    }
  };
  ...
  return (
    <div
      role="region"
      aria-label="Tantangan Logika Roller Coaster"
    ...
    >
      ...
      {!isSuccess && failedAttempts > 0 && (
        <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-xs text-rose-300 flex items-center gap-2 animate-fade-in">
          ...
          <span>Kereta belum mencapai target energi yang diminta.</span>
        </div>
      )}
